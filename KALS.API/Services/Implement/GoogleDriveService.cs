...
public class GoogleDriveService: BaseService<GoogleDriveService>, IGoogleDriveService
{
    public GoogleDriveService(ILogger<GoogleDriveService> logger, IMapper mapper, IHttpContextAccessor httpContextAccessor, IConfiguration configuration) : base(logger, mapper, httpContextAccessor, configuration)
    {
    }
    ...
    public async Task<GoogleDriveResponse> UploadToGoogleDrive(IFormFile file)
    {
        ...
        var credentialsSection = _configuration.GetSection("GoogleDrive:CredentialsPath");
        ...
        try
        {
            ...
            listRequest.Q =
                $"name='{file.FileName}' and '{_configuration["GoogleDrive:FolderId"]}' in parents and trashed=false";
            ...
            {
                Name = file.FileName,
                Parents = new List<string> { _configuration["GoogleDrive:FolderId"] }
            };
